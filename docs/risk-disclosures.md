...
- **A large `redeem` may need splitting into smaller calls.** When the yield sale falls short of the debt slice, `redeem` takes the flash-loan path, which draws the Morpho singleton twice before repaying it once. Where this vault is the dominant collateral supplier that caps one such redemption at ~50% of the position; larger exits revert and must be split, or routed through `redeemInKind`, which never flashes. Rationale in [`architecture.md`](./architecture.md#c-flash-loan-path).
- **`redeemInKind` is not HF-agnostic below HF = 1.** It repays and withdraws pro-rata, so the health factor is preserved rather than improved, and Morpho's check on the withdrawal still rejects an underwater position. It is swap-free, but it is not an exit of last resort — that needs liquidation or emergency recovery to restore health first.
