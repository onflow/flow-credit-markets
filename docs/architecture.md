...
A redemption's debt slice can exceed the yield leg's sale proceeds — a thin pool, an adverse tick, a depegged inner share — leaving the vault short of what it needs to repay debt before collateral can be released. Three ways to close that gap:

|                      | Withdraw collateral (no flash)                                                                                         | Flash loan the debt token                                                                                    | Flash loan the collateral (chosen)                                                                                          |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| Mechanism            | Withdraw the pro-rata collateral directly and accept whatever HF results                                               | Flash-borrow the loan token, repay debt in full, withdraw collateral, sell part of it to repay the flash     | Flash-borrow the redeemer's own collateral, sell part of it to cover the shortfall, repay debt, withdraw to repay the flash |
...
| Works near LLTV      | No — withdrawing before debt is fully repaid would push HF below 1, so Morpho rejects it exactly when it's needed most | Yes — repayment happens before withdrawal, so the withdrawal is HF-neutral                                   | Yes — same HF-neutral ordering                                                                                              |
