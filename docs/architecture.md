...
- `rebalance` enforces this limit per-trade but does **not** revert the whole rebalance when the full target swap would breach it. Instead it swaps the largest feasible fraction under the floor (halving the candidate size until it clears) and lands partway to the re-entry target; successive rebalances converge as price impact shrinks. The per-trade slippage bound — and therefore the per-trade extraction cap — is unchanged; partial filling only changes whether a too-large swap reverts or makes bounded progress. When the slippage is a uniform per-unit cost no smaller trade can escape, the rebalance is a swap-free no-op.
