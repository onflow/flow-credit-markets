...
library MorphoLib {
    ...
    /// @dev Re-exported from `MarketLib` so callers already using `MorphoLib` need only one import; single source of
    /// truth stays in `MarketLib`.
    uint256 internal constant ORACLE_PRICE_SCALE = MarketLib.ORACLE_PRICE_SCALE;
    uint256 internal constant WAD = MarketLib.WAD;
    /// @dev Morpho's share-accounting offsets; used only by the debt conversion below.
    ...
    /// @dev Reads `user`'s collateral from the singleton, then applies `MarketLib.maxBorrowFor` (oracle + LLTV). This
    /// lives here rather than in `MarketLib` because it needs the singleton; `maxBorrowFor` takes the amount directly
    /// and does not.
    /// @param morpho The Morpho Blue singleton.
    /// @param market Morpho market parameters identifying the position.
    ...
    function maxBorrow(IMorpho morpho, MarketParams memory market, address user) internal view returns (uint256) {
        return market.maxBorrowFor(collateral(morpho, market, user));
    }
