...
contract FCMVault is ERC4626, AccessControl, Ownable2Step, IMorphoFlashLoanCallback {
    ...
    function previewDeposit(uint256 assets) public view override returns (uint256) {
        ...
        uint256 feePaid = market.debtToCollateral(toBorrow.mulDiv(feeYieldDebt, SwapLib.FEE_DENOMINATOR));
    ...
    ///         - Case B (`loanGot < d*`): full pro-rata collateral minus the slice
    ///           sold to cover the shortfall `d* - loanGot` (`debtToCollateral`,
    ///           grossed up by `feeAssetDebt`), mirroring `redeem`'s full-value
    ///           flash-loan path — not the old scale-down haircut. Zero when the
    ///           slice is underwater (the sale would exceed the collateral slice).
    ...
    function previewRedeem(uint256 shares) public view override returns (uint256) {
        ...
        uint256 loanValue = yieldSlice.mulDiv(IOracle(yieldOracle).price(), MarketLib.ORACLE_PRICE_SCALE);
        uint256 loanGot = loanValue.mulDiv(SwapLib.FEE_DENOMINATOR - feeYieldDebt, SwapLib.FEE_DENOMINATOR);

        if (loanGot < debtSlice) {
            // Case B: the yield sale falls short of the debt slice. `redeem`
            // flash-borrows the shortfall, repays the full debt slice, withdraws the
            // full collateral slice, and sells just enough of it to cover the
            // shortfall — so the redeemer gets full pro-rata value, not a haircut.
            // The collateral sold is an exact-output swap, so it is grossed up by
            // the asset/debt pool fee. Payout is the collateral slice minus that
            // sale, or 0 if the slice is genuinely underwater.
            uint256 shortfall = debtSlice - loanGot;
            uint256 collSold = market.debtToCollateral(shortfall)
                .mulDiv(SwapLib.FEE_DENOMINATOR, SwapLib.FEE_DENOMINATOR - feeAssetDebt);
            return collSold < collSlice ? collSlice - collSold : 0;
        }
        ...
        returns (uint256)
    {
        ...
        // Band model (main): size to the deposit-target HF (the [min,max] midpoint).
        // Parameterized over (collateralBase, currentDebt) so previewDeposit can pass
        // the simulated post-supply collateral + expected debt. For the live deposit
        // path (collateralBase = current collateral, currentDebt = current debt) this
        // equals market.maxBorrowAtHealthFactor(targetHf).
        uint256 targetHf = _depositTargetHf();
        uint256 capFromNewAsset = market.maxBorrowFor(newAssets).mulDiv(MarketLib.WAD, targetHf);
        uint256 targetDebt = market.maxBorrowFor(collateralBase).mulDiv(MarketLib.WAD, targetHf);
